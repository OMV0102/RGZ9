...
DWORD WINAPI ThreadFunc(void*)
{
	...
	if (library != NULL) // если библиотека загрузилась успешно
	{
		...
		typedef int(*ImportFunction)();  // тип указателя на функцию
		// загрузка функций, определенных в библиотеке
		ImportFunction is_connect = (ImportFunction)GetProcAddress(library, "is_connect");
		ImportFunction is_HT = (ImportFunction)GetProcAddress(library, "is_Hyper_Threading");
